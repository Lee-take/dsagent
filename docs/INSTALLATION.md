...
```text
DS.Agent_1.5.0_x64-setup.exe
```

Version `1.5.0` is the current published stable release. Earlier commits, tags,
...
Release and verify its byte size and SHA-256 against the `v1.5.0` Release before
...
DS Agent `v1.5.0` is intentionally unsigned. Its HTTPS source, immutable tag,
...
publisher, certificate, or signed status is claimed for v1.5.0. If signing is
approved later, it starts with a new version and does not replace this tag or
asset or any immutable v1.1.0, v1.2.0, v1.3.0, or v1.4.0 publication.
...
```powershell
Get-AuthenticodeSignature .\DS.Agent_1.5.0_x64-setup.exe |
  Select-Object Status, StatusMessage, SignerCertificate, TimeStamperCertificate
Get-FileHash .\DS.Agent_1.5.0_x64-setup.exe -Algorithm SHA256
```

For `v1.5.0`, `Status` is expected to be `NotSigned`. An unexpected signer,
...
The v1.5.0 package preserves Kernel-authorized T1 Excel reconciliation and
...
The package also exposes a production durable Computer Use session/step
lifecycle through the right-rail UI and Kernel. Its exact installed reliability
evidence covers generated File Explorer and Excel targets plus installed Edge
using a fresh profile and an ephemeral loopback test portal. That evidence does
not authorize or promise arbitrary websites, stored browser logins, production
portals, secure/UAC desktops, CAPTCHA, payment, or administrator authentication.
...
- Durable steps bind the exact application, process, window, frame, target,
  action, approval, screenshots, and semantic receipt. `ActionStarted` is
  persisted once before an effect; ambiguous results fail closed and are not
  replayed automatically.
- The v1.5.0 File Explorer, Excel, and Edge/loopback cases are installed-test
  evidence for generated isolated targets, not a general browser or arbitrary
  desktop-application support promise.
