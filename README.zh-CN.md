...
<p align="center">
  <a href="https://github.com/Lee-take/dsagent/releases/tag/v1.5.0">v1.5.0 正式稳定版</a> ·
  <a href="https://github.com/Lee-take/dsagent/releases/download/v1.5.0/DS.Agent_1.5.0_x64-setup.exe">下载 Windows 安装包</a> ·
...
在 v1.5.0 中，DeepSeek 可以提出有界 `GoalEnvelope`，但只有本地 Kernel 能校验并冻结它。
...
v1.5.0 安装包保留 T1 Excel/PowerPoint 验证引擎，并新增生产可达的持久 Computer Use
会话/步骤 UI 与 Kernel 闭环。准确安装态证据只覆盖生成的 File Explorer、Excel 目标，以及使用全新 profile 和临时回环测试站点的已安装 Edge；本版本不宣称通用浏览器自动化、复用
已登录状态或生产网站完成能力。普通聊天目前还不会自动选择和串联这两个 T1 工具，也不会自动完成整条 T1 Office 工作流。
...
v1.5.0 仍未开放生产 Microsoft/Google 账号注册和真实外部写入权限。当前正式版不会登录
...
1. 下载 [Windows x64 安装包](https://github.com/Lee-take/dsagent/releases/download/v1.5.0/DS.Agent_1.5.0_x64-setup.exe)。
...
v1.5.0 应用程序和安装包的 Authenticode 状态均为 `NotSigned`。Windows 可能显示
...
DS Agent `v1.5.0` 是如实披露的未签名版本。SignPath Foundation 申请已经提交、仍在等待
审批，本版本不会被描述为已签名。若以后获批，只从后续新版本开始签名，不替换本版本不可
移动的 tag 或资产，也不改写 v1.1.0、v1.2.0、v1.3.0 或 v1.4.0。获准加入该计划的 Release 将遵循：**Free code signing provided by
...
- 安装包：`DS.Agent_1.5.0_x64-setup.exe`
...
- Computer Use：生产可达的持久生命周期与 fail-closed 执行；准确安装态证明仅覆盖生成的
  Explorer/Excel 目标和隔离的 Edge/回环测试站点，不代表通用浏览能力。
...
- [v1.5.0 发布说明](docs/RELEASE_NOTES_v1.5.0.md)
